...
def run_dynamic_analysis(
    ...
    The default ``start_date`` and ``end_date`` arguments are set to
    ``"2023-01-01"`` and ``"2025-01-01"``, respectively, which means
    the analysis window spans **January 2023 through December 2024**.
    If you supply explicit values for these parameters the window will
    adjust accordingly; otherwise the default two‑year horizon is
    applied.  When neither ``start_date`` nor ``end_date`` is set,
    the function falls back to using the final two years of available
    price data.
    ...
    fill_missing_daily : bool, optional
        If ``True``, forward fills missing daily prices before computing
        monthly returns.  This relaxed approach can increase the number
        of DAOs deemed to have complete price data by filling gaps in
        the raw price series.  Defaults to ``False`` (strict mode).
    """
    ...
    # Optionally forward fill missing daily prices to reduce omissions.  This can
    # increase the number of DAOs with complete price data by filling gaps.
    if fill_missing_daily:
        p_window = p_window.ffill()
    ...
    # Print the actual list of DAOs with full price coverage
    print("DAOs with complete price data:")
    print(price_daos)
    ...
    # Print the names of valid DAOs
    print("Valid DAOs:")
    print(valid_daos)
    # Determine which DAOs were omitted despite having full price data
    omitted_daos = [dao for dao in price_daos if dao not in z_df.columns]
    if omitted_daos:
        print("DAOs omitted due to missing z‑scores:")
        print(omitted_daos)
    # Identify DAOs with zero variance in z-scores (dead DAOs)
    dead_daos = [dao for dao in valid_daos if z_df[dao].std(skipna=True) == 0]
    if dead_daos:
        print("DAOs with zero-variance z-scores (potentially dead):")
        print(dead_daos)
    ...
    # Print the monthly timestamps used for the regression.  This helps
    # diagnose off‑by‑one issues where only 23 observations appear.
    print(f"Monthly timestamps used ({len(common_dates)} observations):")
    print(list(common_dates))
    # Compute expected monthly timestamps given the chosen window and log-return labelling
    # Returns are labelled by the first of the next month, so the first expected date is
    # start_date plus one month.  Example: start_date=2023-01-01 -> first label is 2023-02-01.
    if start_date and end_date:
        try:
            start_dt = pd.to_datetime(start_date)
            end_dt = pd.to_datetime(end_date)
            expected_dates = pd.date_range(
                start=start_dt + pd.offsets.MonthBegin(1),
                end=end_dt,
                freq="MS",
            )
            missing_dates = [d for d in expected_dates if d not in common_dates]
            print(f"Expected monthly timestamps ({len(expected_dates)}):")
            print(list(expected_dates))
            if missing_dates:
                print("Missing timestamps (not in common_dates):")
                print(missing_dates)
            else:
                print("No missing timestamps; all expected dates present.")
        except Exception:
